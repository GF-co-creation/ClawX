...
export class GatewayManager extends EventEmitter {
   ...
   * Shared by Gateway spawn and doctor repair.
   */
  private async loadProviderEnv(): Promise<Record<string, string>> {
  ...
  private async runOpenClawDoctorRepair(): Promise<boolean> {
    ...
    const providerEnv = await this.loadProviderEnv();
  ...
  private async startProcess(): Promise<void> {
    ...
    let command: string;
    let args: string[];
    let mode: 'packaged' | 'dev-built' | 'dev-installed' | 'dev-pnpm';
    ...
    if (app.isPackaged) {
      ...
      // Development: prefer installed (packaged) OpenClaw which has compiled
      ...
      if (installedDir) {
        ...
        command = 'node';
        args = [installedEntry, ...gatewayArgs];
    ...
    // Load provider API keys from storage to pass as environment variables
    const providerEnv = await this.loadProviderEnv();
    const loadedProviderKeyCount = Object.keys(providerEnv).length;
