...
export async function sanitizeOpenClawConfig(): Promise<void> {
  ...
  // ── plugins section ─────────────────────────────────────────────
  // plugins.allow with invalid entries (e.g. ["*"]) causes Gateway to
  // reject config entirely.  Remove the key if present.
  const plugins = config.plugins;
  if (plugins && typeof plugins === 'object' && !Array.isArray(plugins)) {
    const pluginsObj = plugins as Record<string, unknown>;
    if ('allow' in pluginsObj) {
      console.log('[sanitize] Removing invalid "plugins.allow" from openclaw.json');
